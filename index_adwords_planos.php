...
<div class="item_anunciante" style="margin-top:20px;">Reportagens:</div>
    ...
    <center>
    <a href="https://www.youtube.com/watch?v=KOrrtxzN4cc" target="new">
    ...
    <a href="http://site.faesa.br/noticia-lendo.aspx?id=689&titulo=Aluno+de+Computa%C3%A7%C3%A3o+cria+aplicativo+para+busca+de+emprego+no+facebook" target="_blank">
        ...
        <div class="opcao_item">
            ...
            <div class="opcao_item">
        	<a href="javascript:void(0)" id="link_anunciantes" style="margin-left:0px;" target="_self">Depoimentos</a>
        </div>
           
             <div class="opcao_item">
        	<a href="javascript:void(0)" id="link_duvidas" target="_self" style="margin-left:20px;">Contato</a>
        </div>
	...
	<a href="javascript:void(0)">
    	<img src="gfx/plano_recrutador_novo/btm_cta_cadastrese.png" alt="cadastrese_agora" />
    </a>
	</div>
<div id="top_header">
...
<?php
//valores padrão
$keyword_dinamica = "Contrate Talentos";
$question = false;
...
<div class="cta">
    ...
    <div class="cta_line_2"></div>
      	...
      	<a href="javascript:void(0)"><img src="gfx/plano_recrutador_novo/cadastresesmall.png" alt="botao cadastrar" class="btm_cadastrar" /></a>
      </div>
</div>
...
<input type="hidden" name="itemCode" value="E6A1C7532C2C37655443CF8F5A32A79A" />
<input type="image" src="gfx/plano_recrutador_novo/planos/empresa_trimestral.png" class="fechabanner" style="border:0;" name="submit" alt="Pague com PagSeguro - é rápido, grátis e seguro!" />
</form>
...
<form action="https://pagseguro.uol.com.br/checkout/v2/cart.html?action=add" method="post" onsubmit="PagSeguroLightbox(this); return false;">
<!-- NÃO EDITE OS COMANDOS DAS LINHAS ABAIXO -->
<input type="hidden" name="itemCode" value="A82CED601B1BF973341F2F829FD73366" />
<input type="image" src="gfx/plano_recrutador_novo/planos/empresa_mensal.png" class="fechabanner"  style="border:0; margin-left:90px;" name="submit" alt="Pague com PagSeguro - é rápido, grátis e seguro!" />
</form>
<script type="text/javascript" src="https://stc.pagseguro.uol.com.br/pagseguro/api/v2/checkout/pagseguro.lightbox.js"></script>
<!-- FINAL FORMULARIO BOTAO PAGSEGURO -->
...
if(isset($_GET['form_contato']))
{
$display_main->show_banner('Vaga Demonstrativa Gratuita','<p>Para provar o nosso potencial e competencia iremos lhe oferecer uma vaga gratuitamente pelo período de 1 dia (ou captação de no máximo 10 currículos).
<strong>Um de nossos consultores irá entrar em contato com você para lhe orientar.</strong></p>


<form action="index_adwords.php" method="post">

<ul style="margin-left:30px;">
<li>Nome Completo: <input type="text" name="nome" value="" placeholder="Seu Nome Completo"/></li>
<li>Empresa: <input type="text" name="empresa" value="" placeholder="Nome da Sua Empresa"</li>
<li>E-mail: <input type="email" name="email" value="" placeholder="E-mail Para Contato"/></li>
<li>Telefone Fixo: <input type="txt" name="telefone" value="" placeholder="(xx) xxxx-xxxx"/> Telefone Celular: <input type="txt" name="celular" value="" placeholder="(xx) x xxxx-xxxx"/></li>
<li></li>
<li>Estado: <input type="text" name="estado" value="" placeholder="Estado"/> Cidade:<input type="text" name="cidade" value="" placeholder="cidade"/></li>
<li></li>
<li>
Descrição da Vaga que deseja divulgar<br />
<textarea name="descricao_vaga" style="width:300px;height:50px;" placeholder="Digite aqui a descrição da vaga que deseja divulgar"></textarea></li>

</ul>

<input type="submit" value="Enviar" class="botao_cta" style="margin-left:30px;"/>
...
if(isset($_POST['descricao_vaga']))//se mandou o nome por post é porque quer orçamento
{
@$nome = mysqli_secure_query($_POST['nome']);
@$empresa = mysqli_secure_query($_POST['empresa']);
@$email = mysqli_secure_query($_POST['email']);
@$telefone = mysqli_secure_query($_POST['telefone']);
@$celular = mysqli_secure_query($_POST['celular']);
@$estado = mysqli_secure_query($_POST['estado']);
@$cidade = mysqli_secure_query($_POST['cidade']);
@$descricao_vaga = mysqli_secure_query($_POST['descricao_vaga']);
...
//faz validação dos campos
    if (checa_vazio(array($nome,$empresa,$email,$telefone,$celular,$estado,$cidade,$descricao_vaga), array('Nome','Empresa','E-mail','Telefone','Celular','Estado','Cidade','Descrição da Vaga'))) 
	{
...
//se chegou até aqui é porque tudo está ok
//vamos inserir o lead na base de dados

$mysqli = mysqli_full_connection();
$mysqli->set_charset('utf8');
$qry = "INSERT INTO leads_empresas VALUES (null, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmt = $mysqli->prepare($qry);
$stmt->bind_param('ssssssss',$nome,$empresa,$email,$telefone,$celular,$estado,$cidade,$descricao_vaga);
$stmt->execute();

if($stmt->affected_rows > 0)
{
$display_main->noty('Obrigado pelo interesse! Sua vaga demonstrativa ficará online por 1 dia e um de nossos consultores irá entrar em contato com você','success','topCenter',6000);

//código do adwords --> conversão
?>
<!-- Google Code for Contato_Empresa Conversion Page -->
...
var google_conversion_label = "uoqpCNva5gkQtbH1zAM";
...
<img height="1" width="1" style="border-style:none;" alt="" src="//www.googleadservices.com/pagead/conversion/966613173/?label=uoqpCNva5gkQtbH1zAM&amp;guid=ON&amp;script=0"/>
</div>
</noscript>
<?php
...
{
	$display_main->noty('Todos os nossos consultores estão ocupados no momento. Por favor, envie um e-mail para sac@empreguemeagora.com.br','error','topCenter',6000);
	exit;
}

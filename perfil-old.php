...
//ajusta botao de contato
        $botao_contato = '';
        $_SESSION['tipo_conta'] = 1; // CORRIGINDO BUG PROVISORIO
...
//esconde nome de usuário se nao for plano recrutador ativo
        ...
        $dados_contato = '';
        $_SESSION['plano_recrutador_ativo'] = 1; //CORRINGINDO BUG MANUAL
        if (isset($_SESSION['plano_recrutador_ativo'])) {
			...
			usu.usu_codigo = ?";
                ...
                while ($stmt->fetch()) {
                    //$dados_contato = '<strong>E-mail: </strong>' . $r_usu_login . '<br /><strong>Telefone Principal: </strong>' . $r_usu_t1 . '<br /><strong>Telefone Secundário: </strong>' . $r_usu_t2 . '<br />';
                    $dados_contato = '<strong>Telefone Principal: </strong>' . $r_usu_t1 . '<br /><strong>Telefone Secundário: </strong>' . $r_usu_t2 . '<br />';
                }
            } else {
                $dados_contato = '';
            }
...
');
    ...
    /*
     * ENVIAR E-MAIL PARA USUÁRIO COM A MENSAGEM DA EMPRESA
     */
    if (isset($_POST['mensagem_candidato']) && $_POST['mensagem_candidato'] == 1) {
        $assunto_email = ($_POST['assunto']);
        $mensagem_email = ($_POST['mensagem']);
        $email_candidato = ($_POST['login']);

        require_once('php_mailer/EmailConfig.php');
        // Setando o endereço de recebimento
        $mail->AddAddress($email_candidato, $usu_nome);
        $mail->AddReplyTo($_SESSION['login']);
        $mail->Subject = $assunto_email;
        $mail->Body = "<p>" . $mensagem_email . "</p><p>Essa mensagem foi enviada pela empresa atráves do <a href='www.empregue-me.com' target='_blank'>empregue-me</a>.</p>";
        if ($mail->send()) {
            $mail->ClearAllRecipients();
            echo "<script type='text/javascript'>alert('Mensagem Enviada com sucesso.');</script>";
            $mysqli7 = mysqli_full_connection();
            mysqli_set_charset($mysqli7, "utf8");
            $qry = "INSERT INTO empresa_mensagem values (NULL,?,?,?,?)";
            $stmt7 = $mysqli7->prepare($qry);
            $stmt7->bind_param('isss', $_SESSION['userid'], $email_candidato, $assunto_email, $mensagem_email);
            $stmt7->execute();
        } else {
            echo "<script type='text/javascript'>alert('Ocorreu um problema ao enviar a mensagem, por favor envie novamente');</script>";
        }
    }
...
//--------------------------------------CÓDIGO DE GERENCIAMENTO DE BANNER DE CONTATO
	...
	$("#resposta_contato").html(data);
		...
		';
                    ...
                    $display_main->show_banner('Contato usuário', '<p>Entre em contato com o candidato enviando uma mensagem para ele:</p><div id="resposta_contato"></div>', 'small');
                }
